...
void set_master_password() {
    ...
    for (int i = 10; i < win_w - 2; ++i) mvwaddch(w, 4, i, ' ');
    wmove(w, 4, 10);
    char confirm[PASS_LEN];
    get_password_masked(w, 4, 10, confirm, PASS_LEN);

    if (strcmp(master, confirm) != 0) {
        mvwprintw(w, 5, 2, "Passwords do not match! Press any key to re-enter...");
        wrefresh(w);
        wgetch(w);
        delwin(w);
        set_master_password();
...
void view_entries() {
    ...
    while (1) {
        ...
        for (int r = 2; r < win_h - 1; ++r) {
            for (int c = 1; c < win_w - 1; ++c) mvwaddch(w, r, c, ' ');
        }
        
        for (int i = 0; i < max_display && (start + i) < entry_count; ++i) {
            int idx = start + i;
            
            mvwprintw(w, 2 + i, 2, "%2d. %-30s : %s", idx + 1, entries[idx].name, entries[idx].password);
        }
        ...
        if (ch == 'q' || ch == 'Q' || ch == 27) break;
            ...
            if (start + max_display < entry_count) start++;
        } else if (ch == KEY_UP) {
            if (start > 0) start--;
        } else if (ch == KEY_NPAGE) { 
            if (start + max_display < entry_count) start += max_display;
            if (start + max_display > entry_count) start = entry_count - max_display;
            if (start < 0) start = 0;
...
void delete_entry() {
    ...
    if (idx >= 1 && idx <= entry_count) {
        ...
        for (int i = idx - 1; i < entry_count - 1; ++i) entries[i] = entries[i + 1];
        entry_count--;
