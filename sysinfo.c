...
int read_cpu_line(const char *line, unsigned long long *idle, unsigned long long *total) {
    ...
    int n = sscanf(line,"cpu%*d %llu %llu %llu %llu %llu %llu %llu",&user,&nice,&system,&iowait,&irq,&softirq,&steal);
    if(n<7) return 0;
    *idle = iowait;
    ...
    while(fgets(buf,sizeof(buf),f)) {
        ...
        if(idx>0) { 
            ...
            if(read_cpu_line(buf,&idle,&total)) {
                ...
                cpu_percent[idx-1] = d_total? (double)(d_total - d_idle)/d_total*100.0:0;
...
void read_mem(double *used, double *total) {
    ...
    while(fgets(line,sizeof(line),f)) {
        ...
        if(sscanf(line,"MemAvailable: %llu kB",&memavail)==1) continue;
    }
    fclose(f);
...
void read_load(double loads[3]) {
    ...
    if(f) { fscanf(f,"%lf %lf %lf",&loads[0],&loads[1],&loads[2]); fclose(f);}
    else loads[0]=loads[1]=loads[2]=0;
}
...
void read_disk(const char *path, double *used, double *total) {
    struct statvfs st;
    if(statvfs(path,&st)!=0) { *used=*total=0; return; }
...
int main() {
    ...
    curs_set(0);
    keypad(stdscr,TRUE);
    if(!has_colors()){ endwin(); printf("No color\n"); return 1; }
    start_color();
    ...
    while(fgets(buf,sizeof(buf),f) && idx<num_cpus) {
        if(strncmp(buf,"cpu",3)==0 && isdigit(buf[3])) read_cpu_line(buf,&prev_idle[idx],&prev_total[idx]);
        idx++;
    }
    fclose(f);
    ...
    while(1) {
        clear();
        mvprintw(0,2,"Fancy System Monitor - Ctrl+C to quit");
        ...
        mvprintw(row,2,"RAM Usage: %.1f / %.1f MB (%.1f%%)",ram_used,ram_total,ram_used/ram_total*100.0);
        draw_bar(row+1,2,50,ram_used/ram_total*100.0,2);
        row +=3;
        double loads[3];
        read_load(loads);
        mvprintw(row,2,"Load Avg: %.2f %.2f %.2f",loads[0],loads[1],loads[2]);
        row +=2;
        ...
        mvprintw(row,2,"Disk / : %.1f / %.1f MB (%.1f%%)",dused,dtotal,dused/dtotal*100.0);
        draw_bar(row+1,2,50,dused/dtotal*100.0,5);
        row +=3;
        read_disk("/home", &dused,&dtotal);
        mvprintw(row,2,"Disk /home : %.1f / %.1f MB (%.1f%%)",dused,dtotal,dused/dtotal*100.0);
        draw_bar(row+1,2,50,dused/dtotal*100.0,5);
